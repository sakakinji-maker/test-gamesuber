...
function stoneLimit(size,color){return color===1?Math.ceil(size*size/2):Math.floor(size*size/2);}
// Captured stones remain spent: on-board stones + stones captured by the opponent.
function remaining(s,color){return Math.max(0,stoneLimit(s.size,color)-s.board.filter(c=>c===color).length-s.captures[other(color)]);}
function play(s,index,enforceSupply=true){
 ...
 if(enforceSupply&&remaining(s,s.turn)===0)throw Error('남은 돌이 없습니다. 패스해 주세요. 잡힌 돌은 보충되지 않습니다.');
...
function resume(s,enforceSupply=true){if(s.phase!=='scoring'||s.result)throw Error('대국을 재개할 수 없습니다.');if(enforceSupply&&!remaining(s,1)&&!remaining(s,2))throw Error('양쪽 모두 돌을 소진했습니다. 계가를 확정해 주세요.');return {...s,phase:'play',dead:[],passes:0};}
function score(s){
...
function suggest(s,level=2,rng=Math.random){
 if(s.phase!=='play'||s.result||!remaining(s,s.turn))return null;const candidates=[];
...
function restore(data){
 ...
 // Older unlimited games may already exceed the new budget. Replay their history
 // without deleting it; subsequent live moves still enforce the remaining supply.
 if(action.type==='play')s=play(s,action.index,false);
 else if(action.type==='dead')s=toggleDead(s,action.index);
 else if(action.type==='resume')s=resume(s,false);
...
return {initialState,stoneLimit,remaining,other,key,coord,neighbors,group,play,toggleDead,resume,score,finish,resign,suggest,restore};
});
