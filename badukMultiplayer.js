...
function attachBaduk(io, { now = Date.now, graceMs = 90000, lobbyTtlMs = 1800000, sweepMs = 1000 } = {}) {
  const nsp = io.of('/baduk');
  const rooms = new Map();
  ...
  function summary() {
    return [...rooms.values()].filter(r => r.status === 'waiting' && r.players.length === 1 && r.players[0].socketId)
      .slice(0, 100).map(r => ({ code: r.code, name: r.players[0].name, createdAt: r.createdAt, size: r.size }));
  }
  function view(room, player) {
    const s = room.state;
    return { code: room.code, size: room.size, scoreReady: room.scoreReady, status: room.status, revision: room.revision, you: player.owner,
  ...
  function end(room, winner, reason) {
    ...
    room.state = { ...room.state, phase: 'finished', result: { winner, reason } };
  }
  function remove(room, player, reason) {
    ...
    if (!room.players.length) rooms.delete(room.code);
      ...
      if (room.status === 'waiting') room.players[0].owner = 1;
      changed(room);
    }
  ...
  function closeRoom(room) {
    for (const p of room.players) if (p.socketId) {
      ...
      if (socket) { socket.data.badukCode = null; socket.emit('roomClosed', '대기 시간이 30분을 넘어 방이 닫혔어요. 새 방을 만들어 주세요.'); }
    }
  ...
  function seated(socket) {
    const room = rooms.get(socket.data.badukCode);
  ...
  function bind(socket, room, player) {
    ...
    socket.data.badukCode = room.code;
  ...
  nsp.on('connection', socket => {
    ...
    handle('create', data => {
      ...
      const room = { code: code(), createdAt: now(), touchedAt: now(), revision: 0,
        status: 'waiting', state: null, records: [], ready: [], scoreReady: [], size: [9,13,19].includes(data.size) ? data.size : 9, players: [] };
      const player = { name, owner: 1, token: randomBytes(32).toString('hex') };
    ...
    handle('join', data => {
      ...
      const player = { name, owner: 2, token: randomBytes(32).toString('hex') };
    ...
    handle('resume', data => {
      ...
      if (previous) { previous.data.badukCode = null; previous.emit('replaced'); previous.disconnect(true); }
      return bind(socket, room, player);
    });
    handle('sync', () => {
      const seat = seated(socket);
    ...
    handle('start', inRoom((room, player) => {
      ...
      room.state = E.initialState(room.size); room.status = 'playing'; room.records = []; room.ready = [];
    ...
    handle('move', inRoom((room, player, data) => {
      ...
      const move = data.move && (data.move.index === null || Number.isInteger(data.move.index)) ? data.move : null;
      if (!move) return fail('올바른 이동 정보가 아니에요.');
      const before = room.state;
      let next;
      try { next = E.play(before, move.index); } catch (_) { return fail('둘 수 없는 수예요. 활로·자충·패·계가 단계를 확인하세요.'); }
      room.records.push({ ply: next.ply, owner: player.owner, text: (player.owner===1?'흑':'백')+' '+(move.index===null?'패스':E.coord(move.index,before.size)) });
      room.state = next;
      if (next.result) room.status = 'finished';
      else if (next.ply >= 4000) { room.state = {...next, phase:'scoring', dead:[]}; }
      room.scoreReady = [];
      changed(room); return { ok: true };
    }, true));
    function scoring(fn) {
      return inRoom((room,player,data)=>{
        if(room.status!=='playing'||room.state.phase!=='scoring')return fail('계가 중인 대국이 아닙니다.');
        if(room.players.length!==2||room.players.some(p=>!p.socketId))return fail('상대의 재접속을 기다립니다.');
        return fn(room,player,data);
      },true);
    }
    handle('dead',scoring((room,player,data)=>{
      try{room.state=E.toggleDead(room.state,data.index);}catch{return fail('돌을 선택하세요.');}
      room.scoreReady=[];changed(room);return {ok:true};
    }));
    handle('accept',scoring((room,player)=>{
      if(!room.scoreReady.includes(player.owner))room.scoreReady.push(player.owner);
      if(room.scoreReady.length===2){room.state=E.finish(room.state);room.status='finished';}
      changed(room);return {ok:true};
    }));
    handle('continue',scoring((room)=>{
      room.state=E.resume(room.state);room.scoreReady=[];changed(room);return {ok:true};
    }));
    ...
    handle('rematch', inRoom((room, player) => {
      ...
      if (room.ready.length === 2) {
        room.state = E.initialState(room.size); room.records = []; room.ready = []; room.scoreReady = []; room.status = 'playing';
      }
      changed(room); return { ok: true };
    }, true));
    handle('leave', inRoom((room, player) => {
      socket.data.badukCode = null; remove(room, player, 'leave');
